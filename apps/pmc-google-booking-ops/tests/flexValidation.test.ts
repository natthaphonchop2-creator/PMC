...
describe('production Flex validation payload', () => {
  it('uses synthetic identity, staff profiles in both messages, and evidence only for Admin', () => {
    ...
    expect(evidenceJson).toContain('สลิป')
    expect(evidenceJson).toContain('แชท 3')
    expect(evidenceJson).not.toContain('แชท 4')
    expect(evidenceJson).not.toContain('แชท 9')
    expect(evidenceJson).toContain('แสดง 4 จากทั้งหมด 10 รูป')
    expect(evidenceJson).not.toContain('"type":"carousel"')
